...
> **These numbers describe the RETIRED five-timestep factorial, not the current sweep.**
> They were measured on the 400 `factorial_*` pickles: `arrows_duration = 5`, the 2x2 over
> oracle gate jitter x `p_corr_by_distance[2]`, 11 signatures, `latent_activation =
> 'softmax'`. The current sweep is `ad10_delay` — 10-timestep trials, no arms, 14
> signatures, and a target-onset delay ladder. **Do not quote a number from this file as a
> property of the current model.**
>
> What survives is the *method*, and it is still what `flanker_rt_threshold_sweep.py` runs:
> the hypotheses H1-H6, the flip table, the decided-only diagnostic, and the finding that
> RT signatures move with the threshold while accuracy ones barely do. The script has been
> ported to the current sweep (`arm` -> `ladder`, thresholds 0.2-0.7); re-run it to get
> numbers for `ad10_delay`, and rewrite this file when you do.
