...
from flanker_figure_utils import (CELLS, COL, band, bars_with_seeds, collect_sessions,
                                  out_dir_for, pretrain_curves, save, series,
                                  sweep_root, _stack, _stack_curve)
...
def _stamp(fig, text, variant, p, n):
    ...
    fig.suptitle(f'{text} — {label}p(congruent)={p}, {n} seeds', fontsize=7)
...
def fig_fingerprint(effects, out_dir, p, variant=DEFAULT_VARIANT):
    ...
    bars_with_seeds(axes[0], [(_stack(effects, f'acc_{k}'), lbl, COL[k]) for k, lbl in CELLS],
                    'Accuracy', baseline=0.5, title='Accuracy by condition')
    bars_with_seeds(axes[1], [(_stack(effects, f'rt_{k}'), lbl, COL[k]) for k, lbl in CELLS],
                    'RT (timesteps)', title='RT by condition')
    bars_with_seeds(axes[2],
                    ...
                    title='Congruency effect by distance')
    bars_with_seeds(axes[3],
                    ...
                    title='Distance effect within congruency')
...
def fig_learning_and_rt(curves, out_dir, p, variant=DEFAULT_VARIANT):
    ...
    for ax, keys, title in [(axes[2], [('cong', 'congruent', COL['cong']),
                                       ('incong', 'incongruent', COL['incong'])], 'RT by congruency'),
                            (axes[3], [('correct', 'correct', COL['correct']),
                                       ('error', 'error', COL['error'])], 'RT by outcome')]:
...
def fig_history(effects, out_dir, p, variant=DEFAULT_VARIANT):
    ...
    bars_with_seeds(axes[3],
                    ...
                    title='Gratton effect vs. repetition')
...
def fig_post_error(effects, out_dir, p, variant=DEFAULT_VARIANT):
    ...
    bars_with_seeds(axes[0],
                    ...
                    'Post-error slowing (RT)', baseline=0.0, title='Post-error slowing')
    ...
    bars_with_seeds(axes[2],
                    ...
                    'Inherited Z focus', connect=True, title='Control state after errors')
    bars_with_seeds(axes[3],
                    ...
                    'Δ Z focus (update)', baseline=0.0, title='What drives the update')
...
def fig_proportion_congruent(out_dir, variant=DEFAULT_VARIANT):
    ...
    label = '' if variant == 'baseline' else f'{variant}, '
    fig.suptitle(f'List-wide proportion congruent — {label}lines are subjects', fontsize=7)
    fig.tight_layout()
...
def main(p=DEFAULT_P, variant=DEFAULT_VARIANT):
